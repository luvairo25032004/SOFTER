...
async def to_know(ctx, command = None,user=discord.Member):
	if not command:
		...
		emb.add_field(name="```=citate MEMBER TEXT```", value = "Оформить сообщение, как цитату")
		...
		emb.add_field(name="```Comittee```", value = "Коммитет по одбору управления сервеpа")
		await ctx.send(embed=emb, delete_after=180)
	elif command == "donate":
...
@Bot.command()
async def citate(ctx,user:discord.Member, * , cnt):
	emb = discord.Embed(description = cnt, colour = 0x00ff80)
	emb.set_footer(text="Автор: {}".format(user.name), icon_url=user.avatar_url)
	await ctx.send(embed=emb)
@Bot.command()
